...
export function fastClassify(query: string): QueryContext | null {
  ...
  const standaloneRealtimeSignal = /\b(live (coverage|updates?|feed|stream|blog|scores?|results?|data)|real.?time (updates?|feed|data|coverage|alerts?|stream)|live\s+\w+\s+(updates?|coverage|feed)|breaking news|latest news|top news|news today|current news|recent news|what('s| is) happening|happening now|news right now)\b/i;
