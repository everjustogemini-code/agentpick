      ...
      schemas: {
        ...
        RouterRequest: {
          ...
          properties: {
            ...
            params: { type: 'object', description: 'Parameters passed through to the tool API. Optional when using flat body (query/q/text/input/url/ticker/symbol at top level).', properties: { query: { type: 'string' }, max_results: { type: 'integer' } } },
            query: { type: 'string', description: 'Flat body shorthand — equivalent to params.query. Omit params when using this.' },
            fallback: { type: 'array', items: { type: 'string' }, description: 'Fallback tool slugs to try if primary fails' },
            strategy: {
              type: 'string',
              enum: ['auto', 'balanced', 'best_performance', 'cheapest', 'most_stable', 'AUTO', 'BALANCED', 'MOST_ACCURATE', 'CHEAPEST', 'FASTEST', 'MANUAL'],
              description: 'Routing strategy. auto (default for new accounts): AI-classifies the query and picks the best tool. balanced: quality × cost-efficiency. cheapest: lowest cost (quality ≥ 3.0). most_stable / FASTEST: highest reliability. best_performance / MOST_ACCURATE: highest benchmark relevance. Overrides the account-level default strategy for this request.',
            },
            priority_tools: {
              type: 'array',
              items: { type: 'string' },
              description: 'Ordered list of tool slugs to try first. Overrides account-level priority_tools and strategy-based selection for this request.',
            },
          },
        },
        RouterResponse: {
          ...
          properties: {
            ...
            meta: {
              ...
              properties: {
                ...
                tried_chain: { type: 'array', items: { type: 'string' }, description: 'All tools attempted in order (primary + fallbacks)' },
                trace_id: { type: 'string' },
                cost_usd: { type: 'number' },
                result_count: { type: 'integer', nullable: true },
                byok_used: { type: 'boolean' },
                strategy: { type: 'string', description: 'Strategy used for this request (SDK enum name)' },
                plan: { type: 'string', description: 'Account plan at time of request' },
                calls_remaining: { type: 'integer', description: 'Calls remaining before hitting daily/monthly limit' },
                message: { type: 'string', nullable: true, description: 'Optional CRM message (upgrade prompts, usage warnings). Null most of the time.' },
              },
            },
          },
        },
