# TASK_CODEX.md — cycle 16

**Agent:** Codex
**Date:** 2026-03-19
**QA baseline:** 50/51 — fixes below bring score to 51/51
**Scope:** Must-Have #1 QA fix + Must-Have #2 UI upgrade (remaining) + Must-Have #3 frontend
...
The following are **confirmed complete** — do not redo:
...
**Status:** `TestEmbedRouter` class does not exist in the file. QA suite is at 50/51 because this test is missing.

Insert the following class **before** the `if __name__ == "__main__":` block at the end of the file (currently line 73):
...
class TestEmbedRouter(unittest.TestCase):
    ...
    def test_b1_embed_tool_used(self):
        ...
        Allowlist is pinned in src/__tests__/router-registry-sync.ts (QA_EMBED_ALLOWLIST).
        """
        r = requests.post(
        ...
        body = r.json()
        valid_embed_tools = ["voyage-embed"]  # must match QA_EMBED_ALLOWLIST in router-registry-sync.ts
...
Also scan the entire file for any existing references to `voyage-ai`, `cohere-embed`, or `jina-embeddings` as embed slugs and remove/replace them (currently zero hits, but verify).
...
**Acceptance:** QA reports **51/51**. `grep "voyage-ai" agentpick-router-qa.py` → 0 results.

---

## Task 2 — Audit `/connect` Page for `voyage-ai` Copy (Must-Have #1)
...
## Task 3 — Complete Glassmorphism + Micro-Animations (Must-Have #2, remaining work)

CSS classes (`glass-card`, `hero-gradient-mesh`, `neon-glow`, `reveal-hidden`, `reveal-visible`, `terminal-cursor`) already exist in `src/app/globals.css`. Apply them where they are still missing.

### 3a — Pricing card glassmorphism
...
### 3b — Hero section: gradient mesh + heading style + gradient underline + cta-glow
...
2. **Hero `<h1>` size**: Find the hero heading and add inline style or Tailwind: `fontSize: 'clamp(3rem, 6vw, 5rem)'`, `lineHeight: 1.08`, `letterSpacing: '-0.03em'`.
...
### 3c — Section scroll reveal (IntersectionObserver)

**File:** `src/app/page.tsx`

Add a single `IntersectionObserver` (in a `useEffect`) that:
- On mount: adds `reveal-hidden` class to each section entry container (feature cards, pricing cards, API carousel section).
...
The CSS transition is already defined in globals.css (`reveal-visible { transition: opacity 400ms ease-out, transform 400ms ease-out }`).

**Constraint:** No animation libraries. Vanilla JS only. Lighthouse perf ≥ 90. CLS < 0.1.

### 3d — Terminal callout upgrade
...
### 3e — Live-stat count-up animation
...
## Task 4 — New `/quickstart` Page + Homepage CTA (Must-Have #3 frontend)

### 4a — Create `/quickstart` page

**File to CREATE:** `src/app/quickstart/page.tsx`

A single page with 3 inline steps. No page reloads between steps. The backend endpoint `POST /api/v1/quickstart/issue` already exists.

**Step 1 — Get a key:**
```tsx
// Email <input> + "Generate free key" <button>
// On submit: POST /api/v1/quickstart/issue with { email, source }
//   - Read ?source from URL searchParams (default: "quickstart")
...
### 4b — Homepage CTA wiring

**File:** `src/app/page.tsx`

1. Find the secondary CTA button/link in the hero section (currently "Try the router" → `href="/connect"` at approximately line 195).
2. Change its label to **"Get started free →"**.
3. Change its `href` to `/quickstart?source=quickstart_homepage`.

The `source=quickstart_homepage` value will be read by the `/quickstart` page and passed to `POST /api/v1/quickstart/issue`, tagging these keys for funnel tracking (backend already supports this).
...
- `src/__tests__/router-registry-sync.test.ts` (Claude Code creates)
...
- `src/lib/benchmark/**`
- `src/lib/ops/**`
- `src/app/globals.css` (already complete, do not modify)
...
| Must-Have #1 — `/connect` page copy: audit `voyage-ai` references | **Task 2 (this file)** |
| Must-Have #1 — CI assertion: `CAPABILITY_TOOLS.embed[0]` pinned to QA allowlist | TASK_CLAUDE_CODE Task A |
| Must-Have #2 — Glassmorphism pricing cards + feature cards | **Task 3a (this file)** |
| Must-Have #2 — Hero animated gradient mesh | **Task 3b (this file)** |
| Must-Have #2 — Hero heading size + line-height + `letter-spacing` | **Task 3b (this file)** |
| Must-Have #2 — Gradient underline on "We fix that." | **Task 3b (this file)** |
| Must-Have #2 — Neon glow CTA pulse (replace shimmer) | **Task 3b (this file)** |
| Must-Have #2 — Sticky nav frosted glass on scroll | already done in SiteHeader ✓ |
| Must-Have #2 — Terminal callout typewriter effect | **Task 3d (this file)** |
| Must-Have #2 — Live-stat count-up on scroll-enter | **Task 3e (this file)** |
| Must-Have #2 — IntersectionObserver fade-up section reveals | **Task 3c (this file)** |
| Must-Have #3 — `/quickstart` page (Step 1 key issue, Step 2 capability pill, Step 3 run) | **Task 4a (this file)** |
| Must-Have #3 — Homepage secondary CTA → "Get started free →" → `/quickstart` | **Task 4b (this file)** |
